...
public class Game {
    ...
    public int doCollide(Item drop){
        if (active.contains(drop)){
            ...
            int itemX = drop.getX();// actual dropping object + 25
            int itemY = drop.getY(); //+ 48
            ...
            System.out.println("item: ("+itemX+" "+itemY+" )");
            System.out.println("tree: "+treeX);
            System.out.println("star: ("+starX+" "+starY+" )");
            System.out.println("distance: "+distance);
            //if (distance < 3) { // do collide
            //  if (drop.isGift()) {
    ...
    public void drop() {
        ...
        if (this.isStart) {
            for (int i = 0; i < 1; i ++) { //active.size() instead of 1
                Item item = active.get(i);
                item.setY(item.getY() + 5);
                if (item.getY() > 740) active.remove(item);
                System.out.println("Item Coord: ["+item.getX()+", "+item.getY()+"]");
                doCollide(item);
            }
        }
